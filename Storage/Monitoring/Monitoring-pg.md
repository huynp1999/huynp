...
| Back_filling    | CRUSH đang trong quá trình tái phân phát lại các PG từ OSD cũ sang mới, hoặc từ OSD bị lỗi sang các OSD còn   lại. | Khi có OSD được thêm hoặc bị xoá khỏi cluster. Khi OSD được thay đổi CRUSH location.                | Ceph tự động, tốc độ backfill có thể tùy chỉnh: `ceph tell 'osd.*' injectargs '--osd-max-backfills 5'`                                                                                                              |
...
| Deep_scrubbing  | Thực hiện checksum các replica của các object, kiểm tra nhất quán                                                  | Thường xuất hiện kèm với trạng thái `inconsistent`                                                       | Ceph tự động                                                                                                               |
